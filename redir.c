...
static ssize_t redir_write(struct redir *r, const char *buf, size_t count)
{
	...
	while(i>0){
		...
		if(i>0){
			...
			if(i>8192) {
				char data[]={0x54,0x00,0x00,0x01,0x00,0x00,0x00,0x00,0x00,0x00,0x20,0x00,0xb4,0x00,0x02,0x00,0x00,0x00,0xb0,0x58,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
				counter=get_counter();
				data[4]=counter%256;
				data[5]=counter/256;
				rc = write(r->sock,&data,IDER_DATA_HEADER_LEN);
					...
					char data[]={0x54,0x00,0x00,0x02,0x00,0x00,0x00,0x00,0x00,0x00,i/2048*8,0x00,0xb5,0x00,0x02,0x00,0x00,i/2048*8,r->buf[14],0x58,0x85,0x00,0x03,0x00,0x00,0x00,r->buf[14],0x50,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
					counter=get_counter();
					data[4]=counter%256;
					data[5]=counter/256;
					rc = write(r->sock,&data,IDER_DATA_HEADER_LEN);
					...
					char data[]={0x54,0x00,0x00,0x03,0x00,0x00,0x00,0x00,0x00,0x00,i/2048*8,0x00,0xb4,0x00,0x02,0x00,0x00,0x00,0xb0,0x58,0x85,0x00,0x03,0x00,0x00,0x00,0xb0,0x50,0x00,0x00,0x00,0x00,0x00,0x00};	
					counter=get_counter();
					data[4]=counter%255;
					data[5]=counter/255;
					rc = write(r->sock,&data,IDER_DATA_HEADER_LEN);
...
int ider_command_handle(struct redir *r){
	...
	if((r->buf[14]==0xb0&&r->buf[15]==0xa0)||(r->buf[14]==0xa0&&r->buf[15]==0xa0)||(r->buf[14]==0x10&&r->buf[15]==0xa0)){
		...
		if(r->buf[12] == 0x00||r->buf[12] == 0xff){
			...
			else request=put_file_size(request, len,r->fd,type);
		}
